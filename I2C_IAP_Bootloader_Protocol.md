...
magic 不匹配? ──→ 参数区未预烧录 ──→ 留在 Bootloader（不做任何初始化）
    ...
    magic 匹配，检查 state:
                        │
    IMAGE_VALID ────────→ 直接跳 APP（信任 state，不复算 CRC）
                        │
    UPDATE_REQUEST ─────→ APP 请求升级 ──→ 停在 Bootloader 等待 IAP
    IMAGE_PENDING ──────→ 前次升级跳 APP 后掉电 ──→ 停在 Bootloader（不重试）
```

> **注意**：`IMAGE_VALID` 由 APP 固件在初始化完成后写入（`BootParam_WriteState(BOOT_PARAM_STATE_IMAGE_VALID)`），不得由 Bootloader 写入。`IMAGE_PENDING` 在掉电重启后不会跳转——这是单次尝试策略，防止升级断电后半完成状态反复跳转失败。
...
| Payload 上限 | 516 字节 | = 4(FlashAddr) + 512(固件) |
| APP_DOWNLOAD 单包固件 | ≤ 512 字节 | 由 HANDSHAKE `FLASH_DATA_MAX` 返回 |
| 每包 PayloadLen | ≤ 516 字节 | = FlashAddr(4) + FLASH_DATA_MAX |
