...
- ERASE_FLASH 内部自动写 UPDATE_REQUEST 防掉电
- CRC_FLASH 写参数区 app_size/app_crc，JUMP_TO_APP 在 Boot 内部闭环验证
- IMAGE_PENDING 默认不重试（BOOT_PENDING_RETRY_MAX = 0）
...
- 五状态：IMAGE_EMPTY(0xFFFFFFFF), UPDATE_REQUEST(0xA5A55A5A), IMAGE_PENDING(0x5AA55AA5), IMAGE_VALID(0x55AAAA55), IMAGE_INVALID(0xDEAD0001)
- boot_count 不递增（避免 Flash 擦写代价），用 IMAGE_PENDING 仅一次尝试
